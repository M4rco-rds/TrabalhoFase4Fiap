  ...
  .addEventListener("submit", function (e) {
    ...
    if (!tipo || !descricao.trim() || isNaN(valor) || valor <= 0) {
      alertaErro.textContent =
        "Preencha todos os campos corretamente com um valor positivo.";

      alertaErro.style.opacity = "1";
      alertaErro.classList.remove("d-none");
      setTimeout(() => {
        alertaErro.style.transition = "opacity 1s ease";
        alertaErro.style.opacity = "0";
        setTimeout(() => alertaErro.classList.add("d-none"), 1000);
      }, 6000);
    ...
    const tabela = document
      .getElementById("tabelaLancamentos")
      .querySelector("tbody");
